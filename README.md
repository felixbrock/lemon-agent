...
3. At the root of the project run `pnpm exec turbo dev` to start the Lemon Agent. In case this doesn't work for you run `pnpm exec turbo build` at the root, run `cd apps/cli` to navigate to the project's cli client, and finally, run `pnpm run dev`.
